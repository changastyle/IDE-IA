...
REM ---------------------------------------------------------------------------
echo.
echo [3/5] Generando CI-CD-LOCAL\version.md ...
...
REM  Version semver: CI-CD-LOCAL\VERSION (MAJOR.MINOR.PATCH)
...
set VERFILE_V=CI-CD-LOCAL\VERSION
...
set VERFILE=CI-CD-LOCAL\version-windows.md
set TMPFILE=CI-CD-LOCAL\_version_new.md
...
if exist "!VERFILE!" (
    findstr /n "^" "!VERFILE!" > CI-CD-LOCAL\_vlines.txt
    set SKIPPED=0
    for /f "usebackq tokens=1* delims=:" %%a in ("CI-CD-LOCAL\_vlines.txt") do (
        set LINE=%%b
        if "%%b"=="---" (
            set /a SKIPPED+=1
        )
...
pyinstaller --noconfirm --windowed --onefile --name "NG-STUDIO" ^
    ...
    --add-data "CI-CD-LOCAL\version-windows.md;CI-CD-LOCAL" ^
...
echo   Version:   !VERSION!
echo   Historial: CI-CD-LOCAL\version-windows.md  (build !BUILD_NUM!)
echo.
