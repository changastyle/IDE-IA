...
# 1 - EL .sh ESTA EN OUT/ PERO LOS ARCHIVOS DEL PROYECTO ESTAN ARRIBA:
cd "$(dirname "$0")/.."
...
# ---------------------------------------------------------------------------
echo
echo "[3/6] Generando OUT/version-linux.md ..."
...
pyinstaller --noconfirm --onefile --name "NG-STUDIO" \
    ...
    --add-data "OUT/version-linux.md:OUT" \
...
echo "  Version:   $VERSION"
echo "  Historial: OUT/version-linux.md  (build $BUILD_NUM)"
echo
